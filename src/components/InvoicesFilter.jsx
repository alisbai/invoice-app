...
export default function InvoicesFilter({ filterOptions, updateFilterBy }) {
  ...
  return (
    <div
      className={`heading-font-s1 invoices-filter ${
        lightSwitch
          ? "invoices-filter-bright-mode"
          : "invoices-filter-dark-mode"
      }`}
      onClick={() => setFilterMenuVisible(!filterMenuVisible)}
    >
      {screenDimensions.width > 768 ? "Filter by status" : "Filter"}
      <img
        className="invoices-filter-down-arrow"
        alt="arrow down"
        src={downArrow}
      />
      <InvoicesFilterMenu
