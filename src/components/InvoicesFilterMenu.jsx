...
export default function InvoicesFilterMenu({visible, filterOptions, updateFilterBy}) {
    ...
    return (
        <div onClick={(e) => e.stopPropagation()} className={`invoices-filter-menu ${visible ? "invoices-filter-menu-visible": "invoices-filter-menu-hidden"} ${lightSwitch ? "invoices-filter-menu-bright-mode" : "invoices-filter-menu-dark-mode"}`}>
            {filterOptionsState.map((filterOption, i) => <div onClick={() => handleClick(i)} className="invoices-filter-option" key={i}><input className="invoices-filter-checkbox" type="checkbox" checked={filterOption.checked}/>{filterOption.option}</div>)}
        </div>
    )
}